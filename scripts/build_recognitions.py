...
# Backward-compatible wrappers for test_recognitions.py (origin/main) and existing consumers.
# These delegate to the quality.py functions defined above.
...
def _engine_confidence_dl(candidate: dict, cid: str = "") -> str:
    ...
    explain_btn = explanation_button("engine_confidence", suffix=cid)
    explain_block = explanation_block("engine_confidence", suffix=cid)
    raw = (
...
def _build_ref_eval_html(candidate: dict, cid: str = "") -> str:
    ...
    explain_btn = explanation_button("reference_evaluation", suffix=cid)
    explain_block = explanation_block("reference_evaluation", suffix=cid)
...
def build_recognition_section(recognitions, doc_id: str, transcript: str,
    ...
    for candidate in candidates:
        ...
        ref_eval_html = _build_ref_eval_html(candidate, cid=cid) if candidate.get("reference_eval") else ""
