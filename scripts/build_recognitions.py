    ...
    try:
        with zipfile.ZipFile(pkg_path, "w", zipfile.ZIP_DEFLATED) as zf:
            ...
            if fused_cand:
                ...
                manifest.append({
                    "file": "fused.txt", "type": "selected",
                    "engine": "fusion", "characters": len(fused_text),
            ...
            for cand in candidates:
                ...
                if cand.get("error"):
                    ...
                    manifest.append({
                        "file": f"candidates/{slug_name}.txt", "type": "candidate",
                        "engine": engine, "model_id": model, "page": page,
                        "characters": len(text),
            ...
            write_exports(directory, doc_id, candidates)
    except OSError:
        return None
    return pkg_path
...
def candidate_json_export(candidate: dict, doc_id: str = "") -> str:
    """Render a candidate as a structured JSON document.

    Includes engine, model, page, confidence, text, and error provenance.
    Selected/fused output is labelled as derivation='selected'.
    """
    engine = candidate.get("engine") or ""
    model = candidate.get("model_id") or ""
    ...
    export = {
        ...
        "characters": len(text) if text and not error else None,
    }
...
def candidate_tei_xml(candidate: dict, doc_id: str = "") -> str:
    """Render a candidate's text as a minimal TEI P5 XML document.

    Identifies machine-generated derivation in the revisionDesc.
    Does not fabricate layout structure absent from the recognition result.
    """
    import xml.sax.saxutils as saxutils
    engine = saxutils.escape(candidate.get("engine") or "unknown")
    model = saxutils.escape(candidate.get("model_id") or "")
    page = saxutils.escape(candidate.get("page") or "")
    text = candidate.get("text") or ""
    date_str = datetime.now(timezone.utc).strftime("%Y-%m-%dT%H:%M:%SZ")
    model_part = f" (model: {model})" if model else ""
    title_parts = [saxutils.escape(doc_id)]
    if page:
        title_parts.append(page)
    title_parts.append(f"{engine}{model_part}")
    title_part = " -- ".join(title_parts)
    para_blocks = []
    for line in text.split("\n"):
        if line:
            para_blocks.append(f"    <p>{saxutils.escape(line)}</p>")
    body_content = "\n".join(para_blocks)
    revision = (
        "  <revisionDesc>\n"
        f"    <change who='#agentic-historian' when='{date_str}'>"
        f"Automated recognition output from {engine}{model_part}</change>\n"
        "  </revisionDesc>"
    )
    return (
        ...
        "    <fileDesc>\n"
        "      <titleStmt>\n"
        f"        <title>{title_part}</title>\n"
        "      </titleStmt>\n"
        "      <publicationStmt>\n"
        "        <p>Machine-generated recognition output. "
        "Not for citation as verified transcription.</p>\n"
        "      </publicationStmt>\n"
        "      <sourceDesc>\n"
        f"        <p>Recognition candidate: engine={engine}, "
        f"model={model}, page={page or 'unassigned'}</p>\n"
        "      </sourceDesc>\n"
        "    </fileDesc>\n"
        f"{revision}\n"
        "  </teiHeader>\n"
...
def write_exports(directory: Path, doc_id: str, candidates: list) -> None:
    """Write per-candidate TEI P5 and JSON structured-export files.

    Exports are stored under rec-exports/<page>/<engine>-<model>.(json|tei.xml).
    Slashes in model IDs are replaced with underscores.
    Only successful candidates get export files; failures get no exports.
    """
    import re
    base = directory / "rec-exports"
    for cand in candidates:
        if cand.get("error") or cand.get("text") is None:
            continue
        engine = re.sub(r"[^A-Za-z0-9._-]", "_", cand.get("engine") or "unknown")
        model = re.sub(r"[^A-Za-z0-9._-]", "_", cand.get("model_id") or "")
        page = re.sub(r"[^A-Za-z0-9._-]", "_", cand.get("page") or "")
        subdir = base / page if page else base
        subdir.mkdir(parents=True, exist_ok=True)
        slug = f"{engine}-{model}" if model else engine
        # TEI P5
        tei = candidate_tei_xml(cand, doc_id)
        (subdir / f"{slug}.tei.xml").write_text(tei, encoding="utf-8")
        # JSON
        json_str = candidate_json_export(cand, doc_id)
        (subdir / f"{slug}.json").write_text(json_str, encoding="utf-8")
...
def build_recognition_section(recognitions, doc_id: str, transcript: str,
    ...
    for candidate in candidates:
        ...
        if not candidate["error"] and artifact_exists:
            eng = quote(candidate["engine"], safe="")
            model = quote(candidate["model_id"] or "", safe="")
            page = quote(candidate["page"] or "", safe="")
            export_links = (
                f'<a class="rec-export rec-export--json" href="rec-exports/{page}/{eng}-{model}.json" download>JSON</a>'
                f'<a class="rec-export rec-export--tei" href="rec-exports/{page}/{eng}-{model}.tei.xml" download>TEI P5</a>'
            )
        else:
            export_links = ""
        if candidate["error"]:
...
<p class="rec-export-links">{export_links}</p></details>''')
    ...
    def _compare_options(all_candidates):
        opts = []
        for c in all_candidates:
            eng = html.escape(c["engine"])
            model = html.escape(c["model_id"]) if c["model_id"] else ""
            page = html.escape(c["page"]) if c["page"] else ""
            label = f"{eng}" + (f" · {model}" if model else "")
            page_hint = f" ({page})" if page else ""
            disabled = " disabled" if c["error"] else ""
            opts.append(
                f'<option value="{html.escape(c["id"], quote=True)}" '
                f'data-page="{page}"{disabled}>'
                f'{label}{page_hint}</option>'
            )
        return "".join(opts)

    compare_options = _compare_options(candidates)
    default_left = candidates[0]["id"] if candidates else ""
    default_right = candidates[1]["id"] if len(candidates) > 1 else default_left
    compare_section = (
        '<div class="rec-compare" data-recognition-compare>'
        '<div class="rec-compare-toolbar">'
        '<button class="btn-rec-compare" type="button" data-rec-compare-open>'
        '&#128269; Vergleichen</button>'
        '</div>'
        '<div class="rec-compare-panes" data-rec-compare-panes hidden>'
        '<div class="rec-compare-pane" data-rec-compare-pane="left" data-rec-compare-selected="{left_id}">'
        '<div class="rec-compare-header">'
        '<label class="rec-compare-label" for="rec-compare-select-left">Version links</label>'
        '</div>'
        '<select class="rec-compare-select" id="rec-compare-select-left" '
        'data-rec-compare-select="left">{compare_options}</select>'
        '<div class="rec-compare-body" data-rec-compare-body="left"></div>'
        '</div>'
        '<div class="rec-compare-pane" data-rec-compare-pane="right" data-rec-compare-selected="{right_id}">'
        '<div class="rec-compare-header">'
        '<label class="rec-compare-label" for="rec-compare-select-right">Version rechts</label>'
        '</div>'
        ...
        '</div></div>'
    ).format(left_id=html.escape(default_left, quote=True),
             right_id=html.escape(default_right, quote=True),
